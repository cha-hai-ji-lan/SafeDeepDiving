...
// 添加小坐标系场景内容
let miniScene: THREE.Scene;  // 小坐标系场景
let miniCamera: THREE.OrthographicCamera;  // 小坐标系相机
let miniRenderer: THREE.WebGLRenderer;  // 小坐标系渲染器
let miniAxesHelper: THREE.AxesHelper;  // 小坐标系坐标轴
...
const init_basic_scenario = () => {
    ...
    // 添加环境光 (基础亮度,防止死黑)
    const ambientLight = new THREE.AmbientLight(0xffffff, 0.5); // 强度 0.5
...
export const init_three = async () => {
    if (!init_basic_scenario()) return  //  初始化基础场景
    init_inline_material()  // 初始化内联材质

    // 初始化小坐标系
    if (!init_mini_coordinate_system()) return
